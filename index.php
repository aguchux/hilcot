...
$Route->add('/pages/{slug}', function ($slug) {
...
$Route->add('/gallery/{slug}', function ($slug) {
...
$Route->add('/info/{slug}', function ($slug) {
...
$Route->add('/news/{slug}', function ($slug) {
...
$Route->add('/brocure/download', function () {
...
$Route->add('/dashboard/{action}', function ($action) {
    ...
    if ($action == "pages") {
        ...
        $Template->assign("galleries", $Core->Galleries());
        $Template->assign("title", "Manage Gallery");
...
$Route->add('/dashboard/pages/{slug}/{action}', function ($slug, $action) {
...
$Route->add('/dashboard/page/{page}/webpart/{webpart}/add', function ($page, $webpart) {
    $Core = new Apps\Core;
    $Template = new Apps\Template("/");
    ...
    $Template->redirect("/dashboard/pages/{$page}/webparts");
}, 'GET');
...
$Route->add('/dashboard/gallery/{photoid}/deletephoto', function ($photoid) {

    $Core = new Apps\Core;
    $Template = new Apps\Template("/");
    $Core->mysqli("DELETE hilc_photos.* FROM hilc_photos WHERE id='$photoid'");
    $Template->redirect("/dashboard/gallery/");
...
$Route->add('/dashboard/sliders/{slide}/{action}', function ($slide, $action) {
...
$Route->add('/dashboard/testimonies/{testimony}/{action}', function ($testimony, $action) {
...
$Route->add('/forms/editpage/homepage', function () {
...
$Route->add('/dashboard/page/{page}/webpart/{part}/delete', function ($page, $part) {
    $Core = new Apps\Core;
    $Template = new Apps\Template;
...
$Route->add('/forms/{action}', function ($action) {
    $Core = new Apps\Core;
    $Template = new Apps\Template;
    $Template->token($Template->data['token']);
    $data = $Core->post($_POST);
    ...
    if ($action == "login") {
    ...
    } elseif ($action == "servicequote") {
        
        $pageid = $data->pageid;
        $Page = $Core->PageInfo($pageid);
        ...
        $EmailTemplate  = new Apps\EmailTemplate("emails.servicequote");

        $EmailTemplate->p_name = $p_name;
        $EmailTemplate->p_email = $p_email;
        $EmailTemplate->p_subject = $p_subject;
        $EmailTemplate->p_message = $p_message;
        $EmailTemplate->p_department = $p_department;
        
        $Mailer = new Apps\Emailer();
        $Mailer->SetTemplate($EmailTemplate);
        $Mailer->subject = "New Quote Request";
        $Mailer->toEmail = $Core->getSiteInfo("email");
        $Mailer->toName = "Hilcot.Com";
        $Mailer->send();

        $Template->redirect("/pages/{$Page->slug}?done=+");

    } elseif ($action == "contactquote") {
        ...
        $EmailTemplate  = new Apps\EmailTemplate("emails.contactquote");
        ...
        $Template->redirect("/info/contact-us?done=+");
        ...
        $EmailTemplate  = new Apps\EmailTemplate("emails.getaquote");
        ...
        $Template->redirect("/pages/get-a-quote?done=+");
        ...
        if ($_FILES["photo"]['size'] > 0) {
            ...
            if ($handle->processed) {
                ...
                $Template->redirect("/dashboard/testimonies/{$testid}/edittestimony");
            }
        }
        $Template->redirect("/dashboard/testimonies/{$testid}/edittestimony");
        ...
        if ($_FILES["photo"]['size'] > 0) {
            ...
            if ($handle->processed) {
                ...
                $Template->redirect("/dashboard/testimonies/{$testimony}/edittestimony");
            }
        }
        $Template->redirect("/dashboard/testimonies");
    } elseif ($action == "addpage") {
        ...
        $Template->redirect("/dashboard/sliders/");
   
    } elseif ($action == "deletephoto") {

        $galleryid = $data->galleryid;
        $Core->mysqli("DELETE hilc_photos.* FROM hilc_photos WHERE id='$galleryid'");
        $Template->redirect("/dashboard/gallery/");
        ...
        if ($_FILES["photo"]['size'] > 0) {
            ...
            $FileDir = "./_store/gallery/photos/";

            $handle->process($FileDir);
            if ($handle->processed) {
                $photo = $handle->file_dst_pathname;
                $Core->mysqli("INSERT INTO hilc_photos(title,photo) VALUES('$title','$photo')");
                $Template->redirect("/dashboard/gallery/");
                $handle->clean();
            }
        }
        ...
        if ($_FILES["photo"]['size'] > 0) {
            ...
            if ($handle->processed) {
                ...
                $Template->redirect("/dashboard/sliders/{$slider}/editslider");
                $handle->clean();
            }
        }
        $Template->redirect("/dashboard/sliders");
        ...
        $Template->redirect("/dashboard/sliders/{$slide}/editslider");
        ...
        if ($_FILES["photo"]['size'] > 0) {
            ...
            if ($handle->processed) {
                ...
                $Template->redirect("/dashboard/products/{$product}/editproduct");
                $handle->clean();
            }
        }
        $Template->redirect("/dashboard/products");
...
$Route->add('/quickedit/{cmskey}', function ($cmskey) {
    $Core = new Apps\Core;
    $data = $Core->post($_POST);
    $text = $data->cmskey;    
...
$Route->add('/quickupload/{cmskey}/w/{w}/h/{h}', function ($cmskey,$w,$h) {
    $Core = new Apps\Core;
